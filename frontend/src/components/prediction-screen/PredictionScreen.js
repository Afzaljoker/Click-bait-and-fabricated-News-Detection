...
const PredictionScreen = ({ newsHeader, similarity }) => {
  ...
  const handleCheckGoogle = async () => {
    try {
      ...
      const config = {
        method: 'get',
        url: `${NEWS_API}/v2/everything?q=${newsHeader}&from=2022-11-1&sortBy=publishedAt&apiKey=${API_KEY}`,
      };
  ...
  return (
    <Container>
      <Box>
        {similarity > 0.6 ? (
          <>
            <Heading>Click-Bait Validation : {similarity}</Heading>
            <h1> Rate is less than 0.6 = Click-bait fake news</h1>
            {googleNews && (
              <p
                style={{
                  cursor: 'pointer',
          ...
          <>
            <Heading>Click-Bait Validation : {similarity} </Heading>
            <h1> Rate is less than 0.6 = Click-bait fake news</h1>
            <Heading>Fabricated news : </Heading>
            {googleNews && (
              <p
                style={{
                  cursor: 'pointer',
